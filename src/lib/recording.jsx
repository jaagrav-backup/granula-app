...
export function RecordingProvider({ children }) {
  ...
  const startRecording = useCallback(async (meeting) => {
    ...
    if (!keys.deepgram) {
      setStatus({ text: 'Add a Deepgram key in Settings first', type: 'idle' })
      return
    }
    ...
    const seeded = (meeting.transcript || []).map((t, i) => ({ id: i + 1, ...t }))
    ...
    nt.on('status', (next) => {
      if (next === 'connecting') setStatus({ text: 'Connecting…', type: 'working' })
      else if (next === 'recording') setStatus({ text: 'Recording', type: 'recording' })
      else if (next === 'stopped') setStatus({ text: 'Stopped', type: 'idle' })
  ...
  const stopRecording = useCallback(async () => {
    ...
    const nt = notetakerRef.current
    if (nt) {
      try { await nt.stopRecording() } catch (err) { console.error(err) }
      notetakerRef.current = null
    }
  ...
  const regenerateNotes = useCallback(async (meeting) => {
    setGenError('')
    const keys = await getKeys()
    if (!keys.gemini) {
